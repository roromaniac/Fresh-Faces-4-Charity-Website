...
def main():
    ...
    This routine forcibly resets the eligibility database on every call. Any existing database
    file is deleted before re-creating it from scratch. If the file cannot be deleted (e.g. due to lock),
    an exception will be raised and MUST be investigated, as this means a stuck or mishandled process.
    """
    # Always reset: delete database file if it exists.
    if os.path.exists(DB_FILE):
        try:
            os.remove(DB_FILE)
            print(f"🗑️  Deleted old DB: {DB_FILE}")
        except Exception as e:
            raise RuntimeError(
                f"❌ Unable to delete {DB_FILE}. This means the SQLite database file is locked or protected. "
                "No other process should be holding it open at this point. Please investigate."
            ) from e

    # (Fresh) connect: will create new file
    ...
    # Execute the eligibility.sql script to set/update eligibility logic
